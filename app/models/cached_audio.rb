class CachedAudio < ApplicationRecord
  ...
  def self.fetch(voice_id:, text:, &block)
    key = Digest::SHA256.hexdigest("#{voice_id}:#{text}:turbo_v2_5")

    record = find_by(cache_key: key)
    ...
    Rails.logger.warn "AudioCache miss (#{e.message}) — generating without cache"
    block.call
  end
end
